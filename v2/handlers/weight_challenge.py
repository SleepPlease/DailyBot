...
async def cmd_weight(message):
    builder = InlineKeyboardBuilder()
    builder.add(
        InlineKeyboardButton(text="Add Weight", callback_data="weight.add_value")
    )
    builder.add(
        InlineKeyboardButton(text="Change Goal", callback_data="weight.change_goal")
    )
    builder.add(
        InlineKeyboardButton(text="Show Graph", callback_data="weight.show_graph")
    )
...
#     kb = [
#         [KeyboardButton(text="..")],
#         [KeyboardButton(text="Add Weight", callback_data="weight.add_weight")],
#         [KeyboardButton(text="Change Goal", callback_data="weight.change_goal")],
#         [KeyboardButton(text="Show Graph", callback_data="weight.show_graph")],
#     ]
#     keyboard = ReplyKeyboardMarkup(
#         keyboard=kb,
#         resize_keyboard=True,
#     )
#     await message.answer("[Weight] Choose an option", reply_markup=keyboard)
#

# https://stackoverflow.com/questions/69846020/how-to-wait-for-user-reply-in-aiogram


@router.callback_query(F.data == "weight.add_value")
...
async def add_value_processor(message, state):
    ...
    try:
        ...
        await message.answer("[Weight] Invalid value. Please try again using numbers.")
        return
    if value <= 0:
        await message.answer("[Weight] Invalid value. Please try again using positive number.")
        return
    ...
    await message.answer("[Weight] Weekly value has been added successfully.")
    await state.clear()


@router.callback_query(F.data == "weight.change_goal")
...
async def change_goal_processor(message, state):
    ...
    await message.answer("[Weight] Goal has been changed successfully.")
    await state.clear()


@router.callback_query(F.data == "weight.show_graph")
